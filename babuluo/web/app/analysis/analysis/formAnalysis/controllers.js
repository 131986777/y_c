...
angular.module('AndSell.Main').controller('analysis_analysis_formAnalysis_Controller', function ($scope, $stateParams, analysisFactory, modalFactory) {
    ...
    function getFormSource(startDay,endDay) {
        analysisFactory.getOfflineFormChangeByRange(startDay,endDay).get({},function (response) {
            ...
            var money_count = 0;
            var order_count = 0;
            var avg_money_count = 0;
            var total_card_money = 0;
            var pt_card_money = 0;
            var vip_card_money = 0;
            var avg_order_money = 0;
            var avg_unit_money = 0;
            var total_unit = 0;
            ...
            for(var j=0;j<(response.data).length;j++){
                ...
                for(var i=0;i<json.length;i++){
                    ...
                    avg_money_count += parseInt(json[i]['SHOP_VALUE']['AVG_MONEY_COUNT'])/100;
                    ...
                    avg_unit_money += parseFloat(json[i]['SHOP_VALUE']['AVG_UNIT_MONEY'])/100;
                    total_unit += parseFloat(json[i]['SHOP_VALUE']['TOTAL_UNIT']);
                    money_discount += parseInt(json[i]['SHOP_VALUE']['MONEY_DISCOUNT'])/100;
                    avg_order_money += parseFloat(json[i]['SHOP_VALUE']['AVG_ORDER_MONEY'])/100;
                ...
                if(j!=0){
                    for(var i=0;i<flag.length;i++){
                        ...
                        flag[i]['SHOP_VALUE']['AVG_MONEY_COUNT'] = parseInt(flag[i]['SHOP_VALUE']['AVG_MONEY_COUNT'])+parseInt(json[i]['SHOP_VALUE']['AVG_MONEY_COUNT']);
                        ...
                        flag[i]['SHOP_VALUE']['AVG_ORDER_MONEY'] = parseFloat(flag[i]['SHOP_VALUE']['AVG_ORDER_MONEY'])+parseFloat(json[i]['SHOP_VALUE']['AVG_ORDER_MONEY']);
                        flag[i]['SHOP_VALUE']['ORDER_COUNT'] = parseInt(flag[i]['SHOP_VALUE']['ORDER_COUNT'])+parseInt(json[i]['SHOP_VALUE']['ORDER_COUNT']);
                        flag[i]['SHOP_VALUE']['AVG_UNIT_MONEY'] = parseFloat(flag[i]['SHOP_VALUE']['AVG_UNIT_MONEY'])+parseFloat(json[i]['SHOP_VALUE']['AVG_UNIT_MONEY']);
            ...
            $scope.AVG_MONEY_COUNT = avg_money_count;
            $scope.TOTAL_CARD_MONEY = total_card_money;
            $scope.PT_CARD_MONEY = pt_card_money;
            $scope.VIP_CARD_MONEY = vip_card_money;
            $scope.AVG_ORDER_MONEY = avg_order_money;
            $scope.AVG_UNIT_MONEY =avg_unit_money;
