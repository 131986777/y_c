...
var AndSellPCMainModule = angular.module('AndSell.PC.Main', ['AndSell.Service', 'AndSell.Router', 'AndSell.UI']);
...
AndSellUI.service('modalFactory', function ($rootScope) {
    ...
    this.setSide = function (state) {
        var bar = {};
        bar.OnOffState = state;
        $rootScope.$broadcast("side-bar", bar);
    };

    this.setCateGory = function (state) {
        var bar = {};
        bar.OnOffState = state;
        $rootScope.$broadcast("categories-bar", bar);
    };
