  ...
  Datetime.prototype = {
    ...
    getConfig: function() {
      var today = new Date(),
        ...
        function getValidTime(today) {
          if (params.min) {
            var minTime = new Date(params.min);

            return minTime < today ? today : minTime;
          } else {
            return today;
          }
        }

        validTime = getValidTime(today);

        var config = {
            rotateEffect: false,  //为了性能
            cssClass: 'datetime-picker',

            value: [validTime.getFullYear(), formatNumber(validTime.getMonth()+1), formatNumber(validTime.getDate()), formatNumber(validTime.getHours()), (formatNumber(validTime.getMinutes()))],
